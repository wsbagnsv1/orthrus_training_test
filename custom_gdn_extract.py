...
def extract_gdn_states_at_positions_kernel(
    ...
    for i in tl.range(0, T):
        ...
        # Check if the current token is an anchor
        anchor_idx = tl.load(anchor_mask + i)
        if anchor_idx >= 0:
...
def extract_gdn_states(
    ...
    Args:
        ...
        beta: (B, T, HV)
        anchor_mask: (T,) tensor mapping token index to anchor_idx, or -1 if not an anchor.
                     Must be int32.
    ...
    assert anchor_mask.shape[0] == T, "anchor_mask must have size T"
